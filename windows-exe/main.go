...
//
// 중요(2026-09-15 사고 정정): 이 아래에서 embed하는 "vehicle-management.html"은 반드시
// build.sh/build-blank.sh가 vehicle-management-demo.html(마스킹판)을 복사해서 만들어둔
// 것이어야 한다 — index.html 시드와 달리, 실제 vehicle-management.html은 회사 실제
// 개인정보(실명·연락처·계좌번호)가 그대로 든 파일이다("임의 생성된 테스트 데이터"라고
// 적혀 있던 이전 주석은 틀린 설명이었고, 그 오해 때문에 실제로 82개 커밋에 걸쳐 이
// Public 저장소에 개인정보가 exe로 유출됐었다). 이 폴더의 vehicle-management.html을
// 절대 직접 실제 파일로 바꿔치기한 채로 커밋하지 말 것.
//
