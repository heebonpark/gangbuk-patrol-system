...
cp ../map-check-manifest.json ./map-check-manifest.json
cp ../map-check-sw.js ./map-check-sw.js
...
# kakao-map.html/kakao-map-admin.html/map-check.html, subscription-dashboard.html은
# 시드 데이터가 아예 없어(CSV/엑셀을 그 자리에서 업로드하는 구조) 배포판에 그대로 담아도 된다.
